from llm_helper import LLMSummarizer
...
class GPTHelper(LLMSummarizer):
    ...
    def check_model(self, model_type:str):
        ...
        if self.model.startswith("gpt-4") and self.model != model_choice["gpt-4"]:
            logging.warning(f"model {self.model} is not supported, will use {model_choice['gpt-4']} instead")
            self.model = model_choice["gpt-4"]
        elif self.model != model_choice["default"]:
            logging.warning(f"model {self.model} is not supported, will use {model_choice['default']} instead")
...
if __name__ == "__main__":
    ...
    recommender = GPTHelper(api_key=api_key, base_url=base_url, model_config=model_config)
    ...
    flag, summarized_text = recommender.request_text_api(text, system_messages="you are a helpful assistant", response_only=True)
